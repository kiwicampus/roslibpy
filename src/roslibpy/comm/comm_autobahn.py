...
class AutobahnRosBridgeProtocol(RosBridgeProtocol, WebSocketClientProtocol):
    ...
    def onMessage(self, payload, isBinary):
        if isBinary:
            ...
            ascii_payload["msg"]["data"] = list(
                struct.unpack(
                    f">{len(ascii_payload['msg']['data'])}B",
                    ascii_payload["msg"]["data"],
                )
            )
            # print(ascii_payload)
    ...
    def onClose(self, wasClean, code, reason):
        LOGGER.info(
            "WebSocket connection closed: Code=%s, Reason=%s", str(code), reason
        )
...
class AutobahnRosBridgeClientFactory(
    EventEmitterMixin, ReconnectingClientFactory, WebSocketClientFactory
):
...
class TwistedEventLoopManager(object):
    ...
    def blocking_call_from_thread(self, callback, timeout):
        ...
        if timeout:
            result_placeholder.addTimeout(
                timeout, reactor, onTimeoutCancel=self.raise_timeout_exception
            )
